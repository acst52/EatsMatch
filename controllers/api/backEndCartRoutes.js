...
// REWORKING ABOVE ROUTE FOR api/cart/add!!!

router.post('/api/cart/add', withAuth, async (req, res) => {
	try {
		// Check if the dish is already in the cart
		let cartItem = await Cart.findOne({
			where: { user_id: req.session.user_id, dish_id: req.body.dishName },
		});
		...
		res.status(500).send('Cannot add to cart. Server error.');
	}
});
